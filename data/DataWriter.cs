...
namespace CharaReader.data
{
	public class DataWriter
	{
		...
		public void WriteStructs<T>(int id, T[] objs) where T : struct
		{
		...
		/// This method buffers each <see cref="Description.description"/> into a single array using <see cref="Description.ptrs"/>
		/// as the offsets for each section of data.
		/// </summary>
		...
		public void WriteDescriptions(string base_name, Description[] descriptions, dynamic separator, int alignment = sizeof(int))
		{
			...
			for (int i = 0; i < descriptions.Length; i++)
			{
				...
				for (int j = 0; j < descriptions[i].ptrs.Length; j++)
				{
					...
					int end;
					for (end = index; end + separator_size < descriptions[i].description.Length; end++)
					{
